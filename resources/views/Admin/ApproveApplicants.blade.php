...
@section('section')
    <main class="flex-1 p-6 overflow-x-auto">
        <div class="w-full mt-5 bg-white rounded p-6">
            <h2 class="text-xl font-semibold">Approved Visa Applicants</h2>
            <p class="text-sm text-gray-700 mb-4">{{ count($fetchApprovedApplicants) }} records found</p>
            <form action="{{ route('Visa.ApprovedApplicants') }}" method="get" class="w-full text-sm border flex flex-row space-x-2"
                autocomplete="off">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-max md:w-full">
                    ...
                    @foreach ($fetchApprovedApplicants as $record)
                ...
                <p class="my-5">{{$fetchApprovedApplicants}}</p>
            </div>
        </div>
    </main>
    </div>
